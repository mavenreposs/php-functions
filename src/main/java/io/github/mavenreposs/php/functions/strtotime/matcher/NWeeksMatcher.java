...
public class NWeeksMatcher implements Matcher {

    private final Pattern days = Pattern.compile("[\\-\\+]?\\d+ weeks");

    public NWeeksMatcher() {
    }
    ...
    public Date tryConvert(String input, Date date) {
        if (days.matcher(input).matches()) {
            ...
            calendar.add(Calendar.WEEK_OF_YEAR, d);
